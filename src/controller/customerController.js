...
const createCustomer = async (res, req) => {
    try {
        const { firstName, lastName, mobileNumber, DOB, address, emailID, status } = req.body
        if (!firstName || !lastName || !mobileNumber || !DOB || !address || !emailID || !status) return res.status(400).send({ status: false, message: "Provide all required fields" })
        if (!["ACTIVE", "INACTIVE"].includes(status)) return res.status(400).send({ status: false, message: "Status must be ACTIVE or INACTIVE" })
...
const customerLogin =async (req, res) => {
    try {
        ...
        if(! DOB) return res.status(400).send({ status:false , message : 'Provide a Date of Birth for login in DD-MM-YYYY' })
        const customer = await customerModel.findOne({ emailID: email , DOB: DOB, status: "ACTIVE" })
